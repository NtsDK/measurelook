...
(function(callback){
	...
	function storiesAPI(LocalDBMS, CommonUtils, Errors) {
        ...
        LocalDBMS.prototype.getStoryElectors = function(storyName, callback){
            "use strict";
            callback(null,  this.database.Stories[storyName].electors);
        };
        
        LocalDBMS.prototype.updateElectorsInfo = function(dataKey, value, callback){
            "use strict";
            this.database.Stories[dataKey.storyName].electors[dataKey.votingStation][dataKey.mark] = value;
            callback();
        };
		...
		LocalDBMS.prototype.createStory = function(storyName, callback){
		    ...
		    R.values(this.database.Characters).forEach(function(votingStation){
		        story.electors[votingStation.name] = {
		                total: -1
		        };
		        Constants.timeMarks.forEach(function(timeMark){
		            story.electors[votingStation.name][timeMark] = -1;
		        });
		    });
		    
		    this.database.Stories[storyName] = story;
		    callback();
		};
		// stories
